...
void PoissonCPU::  setRHS(PencilDcmp &rhs)
{

   double c1,c2,c3;
    if ( !SHIFT )
    {
        c1 = ( coords[1] - coords[0] ) / ( nxChunk + 1. );
        c2 = ( coords[3] - coords[2] ) / ( nyChunk + 1. );
...
#pragma acc loop vector
                ...
                {
                    //          P( id, 1, i, j, k, 0 ) = 1.0;
                    P( id, 0, i, j, k, 0 ) = rhs.getValue(id,i,j,k,0)*c3*c3;
                    P( id, 0, i, j, k, 1 ) = rhs.getValue(id,i,j,k,0)*c3*c3;
                }
            }
        }
    }
