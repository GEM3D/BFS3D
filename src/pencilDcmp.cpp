...
PencilDcmp::PencilDcmp( int n0, int n1, int n2, int px, int py )
{
    ...
    nxChunk = nx / p0;
    nyChunk = ny / p0;
    nzChunk = nz / p0;//nChunk set to p0
...
void PencilDcmp::IO( int app, int dir, int aligndir, const char *h5name, const char *xname )
{
    Phdf5 IO;

    IO.writeMultiBlockCellCenter( P, app, dir, aligndir, h5name,xname );
}
    ...
    for ( int k = 0; k < Nz; k++ )
    {
        ...
        for ( int j = 0; j < Ny; j++ )
        {
            ...
            for ( int i = 0; i < Nx; i++ )
            {
                ...
                rhs[i + j * Nx + Nx * Ny * k] = 4*pi*pi*(cos(2*pi*x)+cos(2*pi*y)) ; /* ( ( omega[0] * omega[0] ) * exactValue( omega[0] * x, tags[0] )
                                                   * exactValue( omega[1] * y, tags[1] ) * exactValue( omega[2] * z, tags[2] )
                                                   + ( omega[1] * omega[1] ) * exactValue( omega[0] * x, tags[0] )
                                                     * exactValue( omega[1] * y, tags[1] ) * exactValue( omega[2] * z, tags[2] )
                                                   + ( omega[2] * omega[2] ) * exactValue( omega[0] * x, tags[0] )
