...
*/
 ...
 //   PoissonCPU *M =new PoissonCPU( argcs, pArgs, Nx, Ny, Nz );
    PoissonCPU *M =new PoissonCPU( Nx, Ny, Nz,2 );
#endif
...
/*
    ...
    M->assignRhs(rhs);
    M->pittPack();
    cout<<"Finished rhs initialization"<<endl;*/
...
//    CFlowVariableSingleBlock Result =U+U ;
   //   U.add(U);
 //   U.P.setValue(Result.P);
//    cout<<"Result value="<<Result.P[10]<<"\n";
    ...
    if ( I_O == 1 )
    {
      ...
      M->IO( 1, dir, 0,"soln/M.h5","soln/M.xdmf" );*/
...
//      pr.Pressure->computeLaplacian();
     ...
     pr.solve(NSTEPS);     
      pr.Veln->getU().IO(1,dir,0, "soln/Pxdmf3d_u100.h5", "soln/Pxdmf3d_u100.xdmf");
         pr.Veln->getV().IO(1,dir,0, "soln/Pxdmf3d_v100.h5", "soln/Pxdmf3d_v100.xdmf");
         pr.Veln->getW().IO(1,dir,0, "soln/Pxdmf3d_w100.h5", "soln/Pxdmf3d_w100.xdmf");
         pr.Pressure->IO(1,dir,0, "soln/Pxdmf3d_p100.h5", "soln/Pxdmf3d_p100.xdmf");
