...
//    void writeMultiBlock( ChunkedArray &F, uint appx ); /*!< writes each element as block and mesh is combination of blocks, appx sets the
  //                                                         appendix as string for the output file  */
    void writeMultiBlockCellCenter( ChunkedArray &F, uint appx, const char *c, const char *x ); /*!< writes each element as block and mesh is combination of blocks, appx
                                                                     sets the appendix as
                                                                     string for the output file  */
    void xdmfMultiBlock( ChunkedArray &F, integer comsize, integer my_rank, uint offset, uint appx, const char *c );
    void xdmfMultiBlockCellCenter( ChunkedArray &F, integer comsize, integer my_rank, uint offset, uint appx, const char *c );
    void writeMultiBlockCellCenter( ChunkedArray &F, uint appx, int dir, int aligndir, const char* c, const char *x );
