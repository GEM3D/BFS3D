...
class PencilDcmp
{
    // private:
    protected:
    const int DATA_WEST=0,DATA_EAST=1,DATA_NORTH=2,DATA_SOUTH=3;
    SignalProc Sig;
    TriDiag    T;
    ...
    public:
    double *getdxyz(){return dxyz;};
    int getMyRank(){return myRank;};
    double *getXbox(){return Xbox;};
    double *getCoords(){return coords;};
    double getValue(int id, int i, int j, int k,int index) ;   
    void setValue(double v, int id, int i, int j, int k,int index) ;   
//    PencilDcmp operator+(const PencilDcmp& b);
    ChunkedArray *getP(){return &P;}; 
    int getnx(){return nx;};
    int getny(){return ny;};
    int getnz(){return nz;};
    int getp0(){return p0;};
    int getnxChunk(){return nxChunk;};
    int getnyChunk(){return nyChunk;};
    int getnzChunk(){return nzChunk;};
...
class CFlowVariable : public PencilDcmp
{
    ...
    //   PencilDcmp getField(){return P;};  
         CFlowVariable( int nx, int ny, int nz, int p0 ) : PencilDcmp( nx, ny, nz, p0,p0 )//constructor without creating grid and coordinates
...
class CVelocitySingleBlock
{
   ...
   public:
         ...
         { U = new CFlowVariableSingleBlock (nx,ny,nz,p0);V = new CFlowVariableSingleBlock (nx,ny,nz,p0);W = new CFlowVariableSingleBlock (nx,ny,nz,p0);
           U->setBox(Xb);U->setCoords(2);  V->setBox(Xb);V->setCoords(2);  W->setBox(Xb);W->setCoords(2);   
           nxChunk=nx/p0; nyChunk=ny/p0; nzChunk=nz/p0; nChunk=p0;p_dir=p0;  Divergence = NULL;
